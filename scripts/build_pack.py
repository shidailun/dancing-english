...
# Jr and Sr are not held: "Mark Maroulis, Jr. For Mr. D who..." is two
# sentences, and "Jr.," mid-sentence never reaches the veto (no space after
# the dot). The Pas de Deux's director is "Mr. D", a name of one capital, so
# "Mr. D. The corners..." breaks after the D: see NAMED below.
ABBREV = r"(?:Mr|Mrs|Ms|Dr|St|Sgt|Lt|vs|etc|Inc|Ave|Rd|Mt|Ft|approx)"
SPLIT = re.compile(
    r"[.!?\u2026]+[\"'\u201d\u2019)\]]*"      # terminator + any closers
    r"\s+"
    r"(?=[\"'\u201c\u2018(\[\u2014]?[A-Z0-9\u201c])"
)
# "No." is held only capitalised (No. 5); "say no. My stop was" is an end.
HOLD = re.compile(r"(?:\b(?i:" + ABBREV + r")|\bNo|\b[A-Z])$")    # text before the dot
# Ends a sentence despite looking like an initial: "Mr. D.", the ballet M+M,
# L.A., D.C., the pronoun I and "a careless X". "Wile E. Coyote" and "Melvin
# C. Washington" are real initials and stay held.
NAMED = re.compile(r"(?:\b(?:Mr|Mrs|Ms|Dr)\. [A-Z]|M\+M|\bL\.A|\bD\.C|\bI|\bX)$")
...
def split_sentences(text):
    ...
    for m in SPLIT.finditer(text):
        head = text[:m.start()]
        if HOLD.search(head) and not NAMED.search(head):   # abbreviation or initial
            continue
...
def build(code, vocab, overrides, seen, report):
    ...
    # These titles are already in the author's case ("Tombé (falling)"), so
    # souls-reader's title_case for its all-caps album titles is not applied.
    b = book_of(code)
    pack = {
        'id': code, 'dialect': 'english', 'group': b['slug'],
        'title': f"{b['title']} · {title}", 'titleTranslation': None,
        'paragraphs': [],
    }
...
def main():
    codes = sys.argv[1:] or all_codes()
    vocab = vocabulary(all_codes())
    ...
    if len(codes) == len(all_codes()):       # a full build owns the registry
        ...
        print(f'\nregistry.json  {len(chapters)} chapters  {total} sentences')
